...
export const properties = (properties?: ThemeProperties): Properties => {
  return {
    radius: {
      'less-round': properties?.radius?.['less-round'] ?? defaultProperties.radius['less-round'],
      round: properties?.radius?.round ?? defaultProperties.radius.round,
      rounder: properties?.radius?.rounder ?? defaultProperties.radius.rounder,
      roundest: properties?.radius?.roundest ?? defaultProperties.radius.roundest,
    },
    shadows: {
      softest: properties?.shadows?.softest ?? defaultProperties.shadows.softest,
